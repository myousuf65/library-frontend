...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        setLoading(true);
        // 1. Get current user (to get studentId)
        ...
        setRecentTransactions(sortedTransactions.slice(0, 5).map(t => ({
          id: t.id,
        ...
        // 4. Calculate currently borrowed books
        const borrowed = [];
        ...
        // Set studentInfo with booksIssued from borrowedBooks.length (after borrowed is initialized)
        setStudentInfo({
          name: student.name,
  ...
  const borrowingCapacity = studentInfo ? (studentInfo.booksIssued / studentInfo.maxBooks) * 100 : 0;

  if (loading) {
    return <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '60vh' }}><Typography variant="h6">Loading dashboard...</Typography></Box>;
  }
  if (error) {

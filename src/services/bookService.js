...
const bookService = {
  getAllBooks: async () => {
    try {
      ...
      console.log(
        "bookService: Response data is array:",
  ...
  createBookWithImage: async (formData) => {
    try {
      ...
      const response = await api.post(
        "/api/books/public/createWithImage",
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  deleteBook: async (id) => {
    try {
      const response = await api.delete(
        `/api/books/public/deleteBook?id=${id}`
      );
      return response.data;
    } catch (error) {
      throw error;
  ...
  getBorrowedBooksByStudent: async (studentId) => {
    try {
      console.log(
        "bookService: Fetching borrowed books for student ID:",
        studentId
      );
      const response = await api.get(
        `/api/books/public/borrowed?studentId=${studentId}`
      );
  ...
  getBookImageUrl: (id) => {
    ...
    }/api/books/public/image?id=${id}&timestamp=${new Date().getTime()}`;
  },
};
